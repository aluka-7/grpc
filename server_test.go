...
import (
	...
	pb "github.com/aluka-7/grpc/testproto"
	nmd "github.com/aluka-7/metacode"
...
var (
	...
	testOnce      sync.Once
	server        *Server
	clientConfig  = ClientConfig{Dial: utils.Duration(time.Second * 10), Timeout: utils.Duration(time.Second * 10), EnableLog: true}
	clientConfig2 = ClientConfig{Dial: utils.Duration(time.Second * 10), Timeout: utils.Duration(time.Second * 10), EnableLog: true,
		Method: map[string]*ClientConfig{`/testproto.Greeter/SayHello`: {Timeout: utils.Duration(time.Millisecond * 200)}},
	}
)
...
func runServer(t *testing.T, interceptors ...grpc.UnaryServerInterceptor) func() {
	return func() {
		server = NewServer(&ServerConfig{Network: "tcp", Addr: testAddr, Timeout: utils.Duration(time.Second), EnableLog: true})
...
func testClientRecovery(t *testing.T) {
	...
	e, ok := errors.Cause(err).(nmd.Codes)
	if !ok {
...
func testErrorDetail(t *testing.T) {
	...
	if ec, ok := errors.Cause(err).(nmd.Codes); !ok {
...
func BenchmarkServer(b *testing.B) {
	server := NewServer(&ServerConfig{Network: "tcp", Addr: testAddr, Timeout: utils.Duration(time.Second)})
	go func() {
...
func TestMetadata(t *testing.T) {
	cli, cancel := NewTestServerClient(func(ctx context.Context, req *pb.HelloRequest) (*pb.HelloReply, error) {
		...
		IdleTimeout:       utils.Duration(time.Second * 180),
		MaxLifeTime:       utils.Duration(time.Hour * 2),
		ForceCloseWait:    utils.Duration(time.Second * 20),
		KeepAliveInterval: utils.Duration(time.Second * 60),
		KeepAliveTimeout:  utils.Duration(time.Second * 20),
		EnableLog:         true,
	}, &ClientConfig{
		Dial:              utils.Duration(time.Second * 10),
		Timeout:           utils.Duration(time.Millisecond * 250),
		KeepAliveInterval: utils.Duration(time.Second * 60),
		KeepAliveTimeout:  utils.Duration(time.Second * 20),
		EnableLog:         true,
	})
...
func TestStartWithAddr(t *testing.T) {
	...
	server = NewServer(&ServerConfig{Network: "tcp", Addr: configuredAddr, Timeout: utils.Duration(time.Second), EnableLog: true})
